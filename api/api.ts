...
import { InitRequestInternalPosition, InitResponse, TrackListEntry } from '../types/init';
...
const retrieveInitDataWithPosition = async (
  ...
  const response = await Backend.put<InitResponse>('/init/app', initRequest, config);
...
const retrieveInitDataWithTrackId = async (
  ...
  const response = await Backend.get<InitResponse>(`/init/app/track/${trackId}`, config);
...
const retrieveUpdateData = async (
  ...
  const response = await Backend.put<UpdateResponse>('/vehicles/app/', updateRequest, config);
...
const retrieveTracks = async (config?: AxiosRequestConfig): Promise<TrackListEntry[]> => {
  const response = await Backend.get<TrackListEntry[]>('/init/app/tracks', config);
